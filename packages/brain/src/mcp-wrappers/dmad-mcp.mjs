...
function exec(cmd) { try { return execSync(cmd, { cwd: REPO, encoding: "utf8", timeout: 60000 }).slice(-2000); } catch (e) { return { error: e.message }; } }
function read(p) { try { return readFileSync(p, "utf8").slice(-3000); } catch (e) { return { error: e.message }; } }
startMcpServer({ serverName: "dmad", serverVersion: "1.0", tools: [
  { name: "run_smoke_test", description: "DMAD 三方辯論 smoke test", inputSchema: { type: "object", properties: {} }, handler: async () => exec("node scripts/dmad-engine/dmad-smoke-test.mjs") },
  { name: "read_calibration", description: "讀 DMAD 校準報告", inputSchema: { type: "object", properties: {} }, handler: async () => read(repoPath("reports", "dmad-calibration.json")) },
  { name: "read_smoke_latest", description: "讀最新 smoke test 結果", inputSchema: { type: "object", properties: {} }, handler: async () => read(repoPath("reports", "dmad-smoke-test-latest.json")) },
]});
